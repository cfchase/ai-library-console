...
const _AppList = ({ aiLibraryApps }) => {
  const filter = null;
  const filteredRows = calculateFilteredRows(filter, aiLibraryApps);
  return (
    <PageSection variant={PageSectionVariants.light} isFilled>
      <TextContent>
        <Text component="h1">AI Library Applications</Text>
        <Text component="p">List of AI Library Applications</Text>
      </TextContent>
      <Table cells={columns} rows={filteredRows} aria-label="List of AI Library Applications">
        <TableHeader />
        <TableBody />
      </Table>
    </PageSection>
...
const mapStateToProps = (state) => {
  return {
    ...
    ...state.appListReducer,
  };
};

const mapDispatchToProps = (dispatch) => ({});

export const AppList = connect(mapStateToProps, mapDispatchToProps)(_AppList);
